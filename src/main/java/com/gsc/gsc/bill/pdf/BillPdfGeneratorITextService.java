...
// Create the table
        Table notesTable = new Table(1);  // Assuming 1 column for simplicity

// Add cell with Arabic content
        String arabicText = "مرحبا بكم في المشروع";  // Sample Arabic text
        PdfFont arabicFont = PdfFontFactory.createFont("C:\\Users\\omarh\\Projects\\Private\\GSC\\GSC\\src\\main\\resources\\font\\NotoNaskhArabic-Regular.ttf", true);
        Cell cell = new Cell()
                .add(arabicText)
                .setFont(arabicFont)
                .setFontSize(12f)
...
//        createDummy();
        ...
        if (billOptional.isPresent()) {
            ...
            table.addCell(new com.itextpdf.layout.element.Cell()
                    .add(new Paragraph("BILL").setFontSize(12f).setBold())
            ...
            if(jobCardOptional.isPresent()) {
                if (jobCardOptional.get().getIsTestDrive() == 1) {
                    document.add(new Paragraph("Test Drive: Yes").setFontSize(12f));
                }
            }
            if (bill.getStatusId() == PAID) {
                document.add(new Paragraph("Status: Paid").setFontSize(12f));
            } else if (bill.getStatusId() == NOT_PAID) {
                document.add(new Paragraph("Status: Not Paid").setFontSize(12f));
            }
            ...
            if (optionalBillProductList.isPresent()) {
