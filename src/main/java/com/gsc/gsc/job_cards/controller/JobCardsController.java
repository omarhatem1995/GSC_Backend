...
public class JobCardsController {
    ...
    @PostMapping("")
    public ResponseEntity addJobCard(@RequestHeader("Authorization") String token,@RequestBody JobCardsDTO jobCardsDTO) {
        return jobCardService.create(token,jobCardsDTO);
    }

    @GetMapping("")
    public ResponseEntity findAllJobCards(@RequestHeader("Authorization") String token , @RequestHeader("Accept-Language") String langId){
        return jobCardService.getJobCardsForUser(token,getLangId(langId));
    }
